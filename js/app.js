...
// bg_48: 48x48 grayscale star (alpha = max(r,g,b)/255), overlay color = ~white. Used for 720p.
// paired_star: 84x84 RGBA star with real colors. Used for 1080p.
...
async function process(buffer, mode, onProgress) {
  ...
  const candidates = margins.map(e => ({
    x: clamp(c - e, 0, c - mask.width),
    y: clamp(u - e, 0, u - mask.height),
  ...
  const finalizeDetection = () => {
    ...
    const margin = 0.04 * Math.max(1, buffered);
    placement = best.score >= def.score + margin ? best : def;

    // estimate opacity by aggregating per-frame scores across buffered frames
    let chosen;
    if (mode === "subtle") chosen = 0.62;
    ...
    // full removal of a grayscale (white) star → push to white + edge cleanup
